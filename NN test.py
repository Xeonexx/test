...
def nonlin(x,deriv=False):          # Created sigmoid as you need to use probability hence only 0-1 graph should be used
    if(deriv==True):                # Used youtube to help with this part, linked in bibliogrpahy above
        return (x*(1-x))
...
X = np.array([[0,0,1],            # Matrix, row is a training/trail example and column should be a different neurone
             [0,1,1],           # 4 training examples, 3 neurons eacxh
             [1,0,1],
             [1,1,1]])
...
#Training
for i in range(50000):             # This should be able to optimse the network for data set, though haven't tested it much and occured 1 error out of 13 attempts.
    l0 = X
